...
const AuthPageComponent = () => {
  ...
  const authPaths = { login: '/auth/login', register: '/auth/register', otp: '/auth/otp' };
  return (
    <div className='flex flex-col'>
      ...
      <div className='bg-whatsapp-light-primary_bg  text-whatsapp-light-text md:min-h-screen  dark:bg-whatsapp-dark-primary_bg dark:text-whatsapp-dark-text'>
        {path === authPaths.login ? <LoginForm /> : path === authPaths.register ? <RegisterForm /> : <OtpVerification />}
      </div>
    </div>
  );
};
