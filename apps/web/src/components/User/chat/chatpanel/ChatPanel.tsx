  ...
  return (
    <div className="bg-pattern h-full w-full" >
      <div ref={parentRef} className="dark:bg-whatsapp-dark-primary_bg flex h-full w-full flex-col bg-[#F5DEB3] bg-opacity-25 dark:bg-opacity-95" >
        <ChatPanelHeader header_name={chat_receiver?.name as string} user_id={chat_receiver?.user_id} for_other receiver_id={chat_receiver?.user_id} chat_id={id} receiver_email={chat_receiver?.user_id} />
