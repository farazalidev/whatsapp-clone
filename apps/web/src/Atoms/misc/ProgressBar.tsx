...
const ProgressBar: FC<ProgressBarProps> = ({ barStyle, isLoading, progress, showDownloadButton, isResumable, onPauseClick, onRetryClick, onDownloadClick, ...props }) => {
  ...
  return barStyle === "circle" ? (
    <div className="w-12 h-12 bg-black rounded-full relative bg-opacity-40 flex justify-center place-items-center">
      ...
      {/* cancel button */}
      {progress && progress < 94 && !isResumable ? <span onClick={onPauseClick} className='z-10 cursor-pointer'><Image src={'/icons/cross.svg'} height={30} width={30} alt='cancel' /></span> : null}

      {/* resume button */}
      {isResumable ? <span onClick={onRetryClick} className='z-10 cursor-pointer'><Image src={'/icons/upload.svg'} height={30} width={30} alt='retry' /></span> : null}

      {/* download button */}
      {showDownloadButton && progress === 100 && !isLoading ? <span onClick={onDownloadClick} className='z-10 cursor-pointer'><Image src={'/icons/gallery-icons/download.svg'} alt='download' height={30} width={30} /></span> : null}
