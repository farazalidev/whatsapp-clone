...
const InputLabelVariants = cva(
  [
    'text-base flex flex-col justify-center text-whatsapp-light-text dark:text-whatsapp-dark-text',
  ],
  {
    variants: {
      labelTextSize: {
      ...
      <input
        ...
        className={cn(
          InputProps({
            ...
            className: `mt-1 ${full_width ? 'w-full' : ''} ${
              error ? 'mb-2' : ''
            } ${
              error
                ? 'outline-red-500'
                : 'outline-whatsapp-default-primary_green outline-'
            } ${className}`,
          })
        )}
        {...props}
